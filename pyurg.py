...
class UrgDevice(serial.Serial):
    ...
    def __receive_data(self):
        return [line.decode('ascii') for line in self.readlines()]
    ...
    # Convenience method for compatibility with our calibration tool
    def get_distance(self):
        """Get distance data from the LIDAR."""
        return self.capture()
    
    # Get total number of scan lines
    def get_scan_lines(self):
        """Get the total number of scan lines."""
        if hasattr(self, 'pp_params'):
            return int(self.pp_params['AMAX']) - int(self.pp_params['AMIN']) + 1
        return 0
    
    # Get sensor information in a dictionary format
    def get_sensor_info(self):
        """Get sensor information as a dictionary."""
        info = {}
        if hasattr(self, 'pp_params'):
            info.update(self.pp_params)
        return info


def main():
    urg = UrgDevice()
    if not urg.connect('/dev/ttyACM0'):
        print('Connect error')
        exit()
